class ProductsController < ApplicationController
    ...
    if (params[:query].present? && params[:category].present?)
      ...
      @products = Product.joins(:category).where(categories: {name: params[:category]}).where('products.name LIKE ?', "%#{params[:query]}%").page(params[:page]).per(20)
    elsif (params[:query].present? && params[:category_name].present?)
      @products = Product.joins(:category).where('products.name LIKE ?', "%#{params[:query]}%").where(categories: {id: params[:category_name]}).page(params[:page]).per(20)
		elsif params[:query].present?
      @products = Product.where('products.name LIKE ?', "%#{params[:query]}%").page(params[:page]).per(20)
    elsif params[:category].present?
    	@products = Category.where("categories.name = ? ", "#{params[:category]}").first.products.page(params[:page]).per(20)
    elsif params[:category_name].present?
      @products = Category.where("id = ?", "#{params[:category_name]}").first.products.page(params[:page]).per(20)
